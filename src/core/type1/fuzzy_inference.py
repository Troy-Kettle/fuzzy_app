import numpy as np
from .fuzzy_fuzzification import fuzzify, fuzzify_non_singleton
...
def run_fuzzy_inference_non_singleton(fis_vars, fis_rules, inputs, input_std_devs, debug_mode=False):
    """
    Run fuzzy inference with non-singleton fuzzification.
    
    Args:
        fis_vars: List of fuzzy variables
        fis_rules: List of fuzzy rules
        inputs: Dictionary of input values
        input_std_devs: Dictionary of standard deviations for input uncertainty
        debug_mode: Whether to show detailed calculations
    
    Returns:
        Tuple of (output_results, rule_trace)
    """
    output_results = {}
    rule_trace = []
    plots = []
    for out_var in [v for v in fis_vars if v['role']=="Output"]:
        ...
        for rule in fis_rules:
            ...
            for vname, sname in rule['if']:
                ...
                # Use non-singleton fuzzification if std_dev is provided
